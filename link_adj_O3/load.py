...
def load_assortative(dataset="cora"):
    ...
    if dataset in ["cora", "citeseer", "pubmed"]:
        ...
        for i in range(len(names)):
            ...
            '''
            with open("/data/liuyue/New/AGE/data/ind.{}.{}".format(dataset, names[i]), 'rb') as rf:
                u = pkl._Unpickler(rf)
                u.encoding = 'latin1'
                cur_data = u.load()
        ...
        test_idx_reorder = parse_index_file(
            "/data/liuyue/New/AGE/data/ind.{}.test.index".format(dataset))
        ...
        labels = labels.numpy()
        return adj, features, labels
    elif dataset in ["amazon-photo", "amazon-computers"]:
        map2names = {
