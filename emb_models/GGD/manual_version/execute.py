...
if __name__ == '__main__':
    ...
    for seed in seeds:
        ...
        setproctitle.setproctitle("GGD-{}-{}".format(args.dataset, seed))
    # for i in range(n_trails):
        ...
        #load dataset
        if dataset in ['cora','citeseer','pubmed', "wiki", "amazon-photo", "amazon-computers", "ogbn-arxiv", "cora-full"]:
            adj, features, labels, idx_train, idx_val, idx_test = process.load_data(dataset)
        elif dataset.rstrip(".npz").split("_")[-1] == "kd":
            adj, features, labels, idx_train, idx_val, idx_test = process.load_kd(dataset)
        else:
        ...
        for epoch in range(nb_epochs):
            ...
            # print(logits_1.shape, lbl.shape)
            # exit(0)

            print('Epoch {}/{} Loss:{}'.format(epoch+1, nb_epochs, loss_disc))
    ...
    #     f.write(str(np.std(acc_results)) + '\n')
        ...
        with torch.no_grad():
            ...
            np.savez("outputs/GGD_{}_emb_{}.npz".format(dataset, seed), emb=emb, labels=labels[0].cpu().numpy())
            ...
            # cluster = SpectralClustering(n_clusters=10, affinity="precomputed", random_state=seed)
