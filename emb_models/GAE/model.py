...
class GAE(nn.Module):
    def __init__(self, gnnlayers, dims, dropout=0.):
        ...
        for i in range(gnnlayers-1):
            ...
            self.layers.append(dnn.GraphConv(dims[i], dims[i+1], activation=F.relu))
...
class VGAEModel(nn.Module):
    def __init__(self, in_dim, hidden1_dim, hidden2_dim, device="cuda:0"):
        ...
        layers = [dnn.GraphConv(self.in_dim, self.hidden1_dim, norm="both", activation=F.relu, allow_zero_in_degree=True),
                  dnn.GraphConv(self.hidden1_dim, self.hidden2_dim, activation=lambda x: x, allow_zero_in_degree=True),
                  dnn.GraphConv(self.hidden1_dim, self.hidden2_dim, activation=lambda x: x, allow_zero_in_degree=True)]
    ...
    def loss(self, adj, adj_rec, pos_weight=None, norm=None):
        device = "cpu"
        ...
        loss = norm * self.loss_function(adj_rec.view(-1), adj.view(-1))
        kl_divergence = 0.5 / adj_rec.size(0) * (
