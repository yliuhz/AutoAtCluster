...
def load_cora_full_diff_cls(nclass=10, seed=None):
    filename = "/data/liuyue/New/SBM/mySBM/data_diff_cls/cora-full_{}_{}_5.npz".format(nclass, seed)
    data = np.load(filename)
...
if __name__ == '__main__':
    ...
    # parser.add_argument("--im_rate", default=0.1, type=float)
    ...
    def get_dataset(path, name, rate=None, seed=None):
    ...
    for rate in rates:
        for seed in seeds:
            ...
            path = osp.join(".", "data")
            dataset = get_dataset(path, args.dataset, rate=rate, seed=seed)

            setproctitle.setproctitle("GRACE-{:.1f}-{}".format(rate, seed))
            ...
            with torch.no_grad():
                ...
                np.savez("outputs/GRACE_{}_emb_{:.1f}_{}_5.npz".format(args.dataset, rate, seed), emb=z)
