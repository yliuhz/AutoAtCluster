...
if __name__ == "__main__":
    ...
    for seed in tqdm(seeds, total=len(seeds)):
        ...
        setproctitle.setproctitle("GS-{}-{}".format(dataset[:2], seed))
        ...
        for epoch in tqdm(range(model_config["epochs"]), total=model_config["epochs"]):
            ...
            if (epoch+1) % 100 == 0:
                with torch.no_grad():
                    ...
                    os.makedirs("OursvsAGE/gs", exist_ok=True) 
                    np.savez("OursvsAGE/gs/{}_{}_{}_{:.1f}_{}_{}_{}.npz".format(dataset, seed, args.lr, args.dropout, epoch+1, args.scaler, args.mlp_layers), data=adj.data, col=adj.col, row=adj.row)
        with torch.no_grad():
            model.eval()
