        ...
        for dataset in datasets:
            ...
            for seed in range(10):
                ...
                if not os.path.exists("Cluster/{}/lo_{}_preds_{}.npz".format(model, dataset, seed)):
                    ...
                    data = np.load("OursvsAGE/{}/{}_{}.npz".format(model, dataset, seed))
                    ...
                    edges = np.arange(num_edges[dataset], min(adj.sum(), 201*num_edges[dataset]), 10*num_edges[dataset])
                    sampling_rates = edges / adj.sum()

                    for idx, sampling_rate in enumerate(sampling_rates):
                        print("m={}".format(edges[idx]))
            #             sampling_rate = m / adj.sum()
                ...
                #         print(nmi, ami, ari)
                        ...
                        np.savez("Cluster/{}/lo_{}_preds_{}_{:d}.npz".format(model, dataset, seed, int(edges[idx])), preds=preds, labels=labels)
                    else:
                        print("Found. Pass.")
                        pass
