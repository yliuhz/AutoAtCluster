...
def gae_for(args, seed):
    ...
    dataset_ = GRACE_datasets[args.dataset]

    data = np.load("/data/liuyue/New/SBM/mySBM/emb_models/GRACE/outputs/GRACE_{}_emb_{}.npz".format(dataset_, seed))
    sm_fea_s = data["emb"]
    dims = [sm_fea_s.shape[-1]] + args.dims
    ...
    with torch.no_grad():
        ...
        hidden_emb = mu.cpu().data.numpy()
        import os
        os.makedirs("outputs_GRACE_emb", exist_ok=True)
        np.savez("outputs_GRACE_emb/AGE_{}_emb_{}.npz".format(args.dataset, seed), emb=hidden_emb, labels=true_labels)
        ...
        # cluster = SpectralClustering(n_clusters=8, affinity="precomputed", random_state=seed)
...
if __name__ == '__main__':
    ...
    for seed in tqdm(seeds, total=args.nexp):
        ...
        setproctitle.setproctitle("AGE-{}-{}".format(args.dataset[:2], seed))
        gae_for(args, seed)