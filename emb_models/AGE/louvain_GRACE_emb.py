    ...
    for model_idx, model in enumerate(models):
        for dataset in datasets:
            ...
            for seed in range(10):
                ...
                if not os.path.exists("Cluster_GRACE_emb/{}/lo_{}_preds_{}.npz".format(model, dataset, seed)):
                    os.makedirs("Cluster_GRACE_emb/{}".format(model), exist_ok=True)

                    labels = true_labels[dataset]
                    m = num_edges[dataset]
                
                    data = np.load("outputs_GRACE_emb/AGE_{}_emb_{}.npz".format(dataset, seed))
                    emb = data["emb"]
            ...
            #             sampling_rate = m / adj.sum()
                ...
                #         print(nmi, ami, ari)
                        ...
                        np.savez("Cluster_GRACE_emb/{}/lo_{}_preds_{}_{}.npz".format(model, dataset, seed, int(edges[idx])), preds=preds, labels=labels)
                    else:
                        print("Found. Pass.")
                        pass
