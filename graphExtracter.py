...
"""
Extract graph structure from attributed graph datasets
"""
...
if __name__ == "__main__":
    ...
    outdir = f"/data/yliumh/AutoAtClusterDatasets/extractStructures"

    for dataset in datasets:
        if not os.path.exists(f"{outdir}/{dataset}.edgelist"):
            print(dataset)
            adj, _, labels = load_data(dataset)
            graph = nx.from_scipy_sparse_array(adj)
            nx.write_edgelist(graph, f"{outdir}/{dataset}.edgelist", delimiter="\t", data=False)
            
            try:
                nk.readGraph(f"{outdir}/{dataset}.edgelist", nk.Format.EdgeListTabZero)
            ...
            # nk.writeGraph(graph_nk,f"{outdir}/{dataset}.gml", nk.Format.SNAP)
        else:
