...
if __name__ == "__main__":
    ...
    for seed in tqdm(seeds, total=len(seeds)):
        ...
        # indices = np.argpartition(-sim, kth=k, axis=-1)[:, :k]
        # adj = np.zeros((emb.shape[0], emb.shape[0]))
        ...
        with torch.no_grad():
            ...
            os.makedirs("link_adj_diff_cls/{}".format(args.model), exist_ok=True)
            np.savez("link_adj_diff_cls/{}/{}_{:d}_{}.npz".format(args.model, args.dataset, args.nclass, seed), data=adj.data, row=adj.row, col=adj.col)
