...
if __name__ == "__main__":
    ...
    for seed in tqdm(seeds, total=len(seeds)):
        ...
        emb_path = os.path.join(emb_paths[args.model], "{}_{}_emb_{:.1f}_{:d}_5.npz".format(args.model, dataset_, args.im_rate, seed))
        data = np.load(emb_path)
        # adj, _, true_labels = load_assortative(dataset=dataset)
        adj, _, true_labels, mask = load_cora_full_im(args.im_rate, seed)
        # adj, _, _ = load_syn_subgraph(dataset=dataset)
        ...
        for epoch in tqdm(range(model_config["epochs"]), total=model_config["epochs"]):
            ...
            if (epoch+1) % 100 == 0:
                with torch.no_grad():
                    ...
                    os.makedirs("gs_im/{}".format(args.im_rate), exist_ok=True) 
                    np.savez("gs_im/{}/{}_{}_{}_{:.1f}_{}_{}_{}_{}_{}.npz".format(args.im_rate, dataset, seed, args.lr, args.dropout, epoch+1, args.scaler, args.mlp_layers, args.pos, args.neg), data=adj.data, col=adj.col, row=adj.row)
