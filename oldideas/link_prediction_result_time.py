...
if __name__ == "__main__":
    ...
    for sampling_edges in edges:
        ...
        for seed in tqdm(seeds, total=len(seeds)):
            ...
            sampling_times.append(ed_sampling-st)
            lo_times.append(ed_lo-ed_sampling)

        sampling_rate = sampling_edges/m
        sampling_time_m[sampling_rate]=np.mean(sampling_times)
        lo_time_m[sampling_rate]=np.mean(lo_times)

    with open("Ours_time.txt", "a+") as f:
        f.write("{}\n".format(args.dataset))
        f.write("sampling_times: {}\n{}\n".format(sampling_time_m, sampling_time_m.values()))
        f.write("lo_times: {}\n{}\n".format(lo_time_m, lo_time_m.values()))
