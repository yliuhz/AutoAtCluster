...
# @profile
def train(out_epochs, in_epochs, pos_k, neg_k, bs, sample_size, inx, emb, model, optimizer, criterion, device):
    n = emb.shape[0]
    best = 1e9

    for super_epoch in tqdm(range(out_epochs), total=out_epochs):
            model.train()
            st, ed = 0, bs
            length = n
            n_batches = int(np.ceil(length/bs))
            cnt = 0
            while ed <= length:
                cnt += 1
                # Cal pos inds
                sub_n = ed-st
                sub_emb = emb[st:ed]
                sim = np.matmul(sub_emb, np.transpose(emb))
                pos_indices = np.argpartition(-sim, kth=pos_k, axis=-1)[:, :pos_k]
                mask = np.tile(np.transpose(np.arange(st, ed)[np.newaxis]), pos_k)
                pos_indices = mask * n + pos_indices
                pos_inds = torch.LongTensor(pos_indices.reshape(-1))


                # negsize = (ed-st) #TIPS: 与正样本个数相同
                # node_ids = np.random.RandomState(seed=seed).permutation(n)[:sub_n]
                # sub_emb = emb[node_ids]
                # sim = np.matmul(sub_emb, np.transpose(emb))
                neg_indices = np.argpartition(sim, kth=neg_k, axis=-1)[:, :neg_k]
                mask = np.tile(np.transpose(np.arange(st, ed)[np.newaxis]), neg_k)
                neg_indices = mask * n + neg_indices
                neg_inds = torch.LongTensor(neg_indices.reshape(-1))

                num_samples = sub_n * pos_k
                for epoch in range(in_epochs):

                    sampled_neg = torch.LongTensor(np.random.choice(neg_inds.numpy(), size=num_samples))
                    sampled_inds = torch.cat((pos_inds, sampled_neg), 0).to(device)
                    xinds = sampled_inds // emb.shape[0]
                    yinds = sampled_inds % emb.shape[0]
                    x = torch.index_select(inx, 0, xinds).to(device)
                    y = torch.index_select(inx, 0, yinds).to(device)
                    labels = torch.cat([torch.ones(num_samples), torch.zeros(num_samples)]).to(device)
                    inputs = (x * y).to(device)
                    ...
                    tqdm.write("EPOCH {:d} {:d}/{:d} {:d}/{:d} Loss:{:.4f}".format(super_epoch+1, cnt, n_batches, epoch+1, in_epochs, loss.item()))

                if loss < best:
                    best = loss
                    torch.save(model.state_dict(), "saved_models/GGD_ogbn_products_best.pkl")

                st = ed
                if ed < length and ed + sample_size >= length:
                    ed += length - ed
                else:
                    ed += sample_size
    
            # if epoch % 10 == 0:
            #     tqdm.write("EPOCH {:d}/{:d} Loss:{:.4f}".format(super_epoch+1, out_epochs, loss.item()))
            ...
            #         print(epoch, nmi, ami, ari, nclass, loss.item())
...
def main():
    ...
    for seed in tqdm(seeds, total=len(seeds)):
        ...
        # # # TIPS: 生成正确的pos和neg
        # # # NOTE: 结果（nclass=0.9）：0.9499410087657156 0.9495080568057495 0.8760800724549285，表示模型没问题
        # # # 结果（nclass=0.1）：0.8586626447956507 0.8560414260129972 0.848494688045577
        ...
        n = adj.shape[0]
        m = adj.sum()
        pos_k = int(args.pos_k_times_m*m / n)
        neg_k = int((n - pos_k)*args.neg)
        emb = normalize(emb)
        ...
        #     adj[i][pos_indices[i]] = 1.
        # plot_superadj(adj, K=100, sparse=False, labels=true_labels, vline=True, dataset="pos_knn")
        # adj = np.zeros((emb.shape[0], emb.shape[0]))
        # for i in range(neg_indices.shape[0]):
        #     adj[i][neg_indices[i]] = 1.
        # plot_superadj(adj, K=100, sparse=False, labels=true_labels, vline=True, dataset="neg_knn")

        # mask = np.tile(np.transpose(np.arange(n)[np.newaxis]), pos_k)
        # pos_indices = mask * n + pos_indices
        # mask = np.tile(np.transpose(np.arange(n)[np.newaxis]), neg_k)
        # neg_indices = mask * n + neg_indices
        # pos_inds = torch.LongTensor(pos_indices.reshape(-1))
        # neg_inds = torch.LongTensor(neg_indices.reshape(-1))

        # exit(0)
        bs = min(model_config["sample_size"], n)
        print(f"pos_k={pos_k:.1f}, neg_k={neg_k:.1f}")
        ...
        tracemalloc.start()
        train(model_config["out_epochs"], model_config["in_epochs"], pos_k, neg_k, bs, model_config["sample_size"], inx, emb, model, optimizer, criterion, model_config["device"])
