...
if __name__ == "__main__":
    ...
    for seed in tqdm(seeds, total=len(seeds)):
        ...
        setproctitle.setproctitle("lkO1-{}-{}-{}".format(args.model, dataset[:2], seed))
        ...
        data = np.load("params_O1/{}-{}.npz".format(args.model, dataset))
        ...
        with torch.no_grad():
            ...
            os.makedirs("link_adj_O1/{}".format(args.model), exist_ok=True)
            np.savez("link_adj_O1/{}/{}_{}.npz".format(args.model, dataset, seed), data=adj.data, col=adj.col, row=adj.row)


            if args.save_model:
                os.makedirs("saved_models_O1", exist_ok=True)
                torch.save(model.state_dict(), "saved_models_O1/{}-{}-{}.statedict".format(args.model, args.dataset, seed))
