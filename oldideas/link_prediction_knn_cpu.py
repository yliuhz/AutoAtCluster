...
# @profile
def train(num_epochs, pos_k, neg_k, sample_size, inx, emb, model, optimizer, criterion, device):
    n = emb.shape[0]
    ...
    # exit(0)
    bs = min(sample_size, len(pos_inds))
    ...
    for epoch in tqdm(range(num_epochs), total=num_epochs):
        ...
        while ed <= length:
            ...
            sampled_inds = torch.cat((pos_inds[st:ed], sampled_neg), 0).to(device)
            ...
            labels = torch.cat([torch.ones(ed - st), torch.zeros(negsize)]).to(device)
        ...
        if epoch % 10 == 0:
            tqdm.write("EPOCH {:d}/{:d} Loss:{:.4f}".format(epoch+1, num_epochs, loss.item()))
...
def main():
    ...
    for seed in tqdm(seeds, total=len(seeds)):
        ...
        tracemalloc.start()
        train(model_config["epochs"], pos_k, neg_k, model_config["sample_size"], inx, emb, model, optimizer, criterion, model_config["device"])
        ...
        with torch.no_grad():
            ...
            # os.makedirs("link_adj_diff_cls_knn/{}".format(args.model), exist_ok=True)
            # np.savez("link_adj_diff_cls_knn/{}/{}_{:d}_{}.npz".format(args.model, args.dataset, args.nclass, seed), data=adj.data, row=adj.row, col=adj.col)
