...
if __name__ == "__main__":
    ...
    for seed in tqdm(seeds, total=len(seeds)):
        ...
        #     adj[x][y] = adj[y][x] = 1.
        # # preds = infomap_cluster(sp.csr_matrix(adj), true_labels, random_state=0)
        ...
        for epoch in tqdm(range(model_config["epochs"]), total=model_config["epochs"]):
            ...
            if epoch % 100 == 0:
                with torch.no_grad():
                    ...
                    preds = infomap_cluster(adj, labels, random_state=seed)
                    labels = true_labels
        ...
        with open("best_params_knn_20.txt", "a+") as f:
            f.write(f"{dataset} {max_epoch:d} {max_ari:.3f}\n")