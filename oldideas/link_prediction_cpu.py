...
if __name__ == "__main__":
    ...
    for seed in tqdm(seeds, total=len(seeds)):
        ...
        print(f"Num of samples:{pos_num:d}")
        ...
        for epoch in tqdm(range(model_config["epochs"]), total=model_config["epochs"]):
            ...
            while ed <= length:
                ...
                times1.append(ed1-st1)
                times2.append(ed2-ed1)
                times3.append(ed3-ed2)
    
            if epoch % model_config["print_epoch"] == 0:
                tqdm.write("EPOCH {:d}/{:d} Loss:{:.4f} T1:{:.2f} T2:{:.2f} T3:{:.2f} NB:{:d}".format(epoch+1, model_config["epochs"], loss.item(), np.mean(times1), np.mean(times2), np.mean(times3), n_batches))
