...
from load import load_assortative, load_cora_full_im, load_cora_full_diff_cls
...
if __name__ == "__main__":
    ...
    for seed in tqdm(seeds, total=len(seeds)):
        ...
        setproctitle.setproctitle("GS-CF-{}".format(args.nclass))
        ...
        # adj, _, true_labels, mask = load_cora_full_im(args.im_rate)
        # adj, _, _ = load_syn_subgraph(dataset=dataset)
        adj, _, true_labels, mask = load_cora_full_diff_cls(args.nclass)
        ...
        for epoch in tqdm(range(model_config["epochs"]), total=model_config["epochs"]):
            ...
            if (epoch+1) % 100 == 0:
                with torch.no_grad():
                    ...
                    os.makedirs("gs_diff_cls/{}".format(args.nclass), exist_ok=True) 
                    np.savez("gs_diff_cls/{}/{}_{}_{}_{:.1f}_{}_{}_{}_{}_{}.npz".format(args.nclass, dataset, seed, args.lr, args.dropout, epoch+1, args.scaler, args.mlp_layers, args.pos, args.neg), data=adj.data, col=adj.col, row=adj.row)
