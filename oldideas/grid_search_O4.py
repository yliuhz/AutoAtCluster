...
if __name__ == "__main__":
    ...
    setproctitle.setproctitle("gs4-{}-{}".format(args.model, dataset[:2]))
    ...
    for seed in tqdm(seeds, total=len(seeds)):
        ...
        # valid_inputs = (x - y).abs().to(device)
        valid_inputs = ((x - y)**2).to(device)
        ...
        os.makedirs("params_O4", exist_ok=True)
        np.savez("params_O4/{}-{}.npz".format(args.model, args.dataset), pos=pos_rs[x], neg=neg_rs[y])
