...
if __name__ == "__main__":
    ...
    for seed in tqdm(seeds, total=len(seeds)):
        ...
        data = np.load("gs_MVGRL/{}_{}_{}_{:.1f}_{}_{}_{}_{}_{}.npz".format(dataset, seed, args.lr, args.dropout, args.epoch, args.scaler, args.mlp_layers, args.pos, args.neg))
        ...
        df_data_nmi.to_csv("Ours_gs_NMI_{}.csv".format(args.model), index=False, mode="a+")
        df_data_ami.to_csv("Ours_gs_AMI_{}.csv".format(args.model), index=False, mode="a+")
        df_data_ari.to_csv("Ours_gs_ARI_{}.csv".format(args.model), index=False, mode="a+")
