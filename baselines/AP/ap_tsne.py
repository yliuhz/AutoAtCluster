...
def ap_cluster(adj, features, nclass, random_state=None, labels=None):
    from community import community_louvain
    import networkx as nx
    ...
    # max_iters = np.arange(200, 1000, 100)
    # for max_iter in max_iters:
    dps = np.arange(0, 1.0, 0.1)
    for dp in dps:
        cluster = AffinityPropagation(damping=dp)
        ...
        print(ari, np.unique(preds).shape)
    ...
    for model in models:
        for dataset in datasets:
            for seed in seeds:
                ...
                setproctitle.setproctitle("AP-{}-{}-{}".format(model, dataset[:2], seed))
                ...
                preds = ap_cluster(adj, emb, nclass, random_state=seed, labels=labels)
