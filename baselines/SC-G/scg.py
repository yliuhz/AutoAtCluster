...
def scg_cluster(adj, features, nclass, random_state=None):
    from community import community_louvain
    import networkx as nx
    from sklearn.metrics import normalized_mutual_info_score as NMI, adjusted_mutual_info_score as AMI
    from sklearn.cluster import SpectralClustering

    cluster = SpectralClustering(n_clusters=nclass, random_state=random_state, affinity="precomputed")
    preds = cluster.fit_predict(adj.toarray())
...
if __name__ == "__main__":
    ...
    for dataset in datasets:
        for seed in seeds:
            ...
            setproctitle.setproctitle("SCG-{}-{}".format(dataset[:2], seed))

            adj, features, labels, _, _, _ = load_data(dataset)
            nclass = nclasses[dataset]

            preds = scg_cluster(adj, features, nclass, random_state=seed)

            np.savez("outputs/{}_{}.npz".format(dataset, seed), preds=preds, labels=labels)
