...
if __name__ == "__main__":
    ...
    for seed in tqdm(seeds, total=len(seeds)):
        ...
        for sampling_edges in edges:
            ...
            data = np.load("outputs/knn_adj_{}_{}_{:.1f}.npz".format(args.dataset, seed, sampling_edges/m))
            ...
            labels = true_labels
            preds = leiden_cluster(adj, labels, random_state=seed) 
            ...
            os.makedirs("Cluster_leiden/{}".format(args.model), exist_ok=True)
            np.savez("Cluster_leiden/{}/lo_{}_preds_{}_{:.1f}.npz".format(args.model, args.dataset, seed, sampling_edges / m), preds=preds)
