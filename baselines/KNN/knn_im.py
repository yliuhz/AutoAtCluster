...
def load_emb(model, dataset, seed, rate=None, nclass=None):
    ...
    if rate is not None:
        emb_path = os.path.join(emb_path, "{}_{}_emb_{:.1f}_{:d}_5.npz".format(model, dataset_, rate, seed))    
...
if __name__ == "__main__":
    ...
    for model in models:
        for dataset in datasets:
            for rate in rates:
                for seed in seeds:
                    print(model, dataset, rate, seed)

                    np.random.seed(seed)
                    random.seed(seed)

                    setproctitle.setproctitle("KNNim-{:.1f}-{}".format(rate, seed))

                    # adj, features, labels, _, _, _ = load_data(dataset)
                    adj, features, labels, mask = load_cora_full_im(rate, seed)
                    emb = load_emb(model, dataset, seed, rate=rate)
                    # emb = emb[mask]
                    ...
                    # plot_superadj(knn_adj, K=100, sparse=True, labels=labels, dataset="knn_{}_{:.1f}_{:.3f}_5".format(model, rate, ari), vline=True)
                    ...
                    np.savez("outputs_im/knn_adj_{}_{:.1f}_{}_5.npz".format(model, rate, seed), data=knn_adj.data, row=knn_adj.row, col=knn_adj.col)
