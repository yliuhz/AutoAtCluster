...
if __name__ == "__main__":
    ...
    for seed in tqdm(seeds, total=len(seeds)):
        ...
        for nclass in nclasses:
            ...
            adj, features, true_labels, mask = load_cora_full_diff_cls(nclass, seed)
            n = adj.shape[0]
            m = adj.sum()
            raw_adj = adj.copy()

            setproctitle.setproctitle("KNNR-{}-{}".format(nclass, seed))

            data = np.load("outputs_diff_cls/knn_adj_{:d}_{:d}.npz".format(nclass, seed))
            ...
            os.makedirs("Cluster_diff_cls", exist_ok=True)
            np.savez("Cluster_diff_cls/lo_preds_{:d}_{:d}.npz".format(nclass, seed), preds=preds)
