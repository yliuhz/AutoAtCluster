...
if __name__ == "__main__":
    seeds = np.arange(0, 3, dtype=int) # seed = 0
    # seeds = [11]
    # seeds = [7]
    ...
    for dataset in datasets:
        ...
        for idx, sampling_edges in enumerate(edges):
            times = []
            for seed in seeds:
                np.random.seed(seed)
                random.seed(seed)

                print(dataset, idx+1, seed)

                setproctitle.setproctitle("KNNR-{}-{}".format(dataset[:2], seed))

                data = np.load("outputs/knn_adj_{}_{}_{:.1f}.npz".format(dataset, seed, sampling_edges/m))
                ...
                # os.makedirs("Cluster/{}".format(args.model), exist_ok=True)
                # np.savez("Cluster/{}/lo_{}_preds_{}_{:.1f}.npz".format(args.model, args.dataset, seed, sampling_edges / m), preds=preds)
            time_m[sampling_edges/m] = np.mean(times)

        with open("KNN_time.txt", "a+") as f:
            f.write("{}\n".format(dataset))
            f.write("Louvain_times: {}\n{}\n".format(time_m, time_m.values()))
