...
def ms_cluster(adj, features, random_state=None, labels=None):
    from community import community_louvain
    import networkx as nx
...
if __name__ == "__main__":
    ...
    for dataset in datasets:
        ...
        for seed in seeds:
            ...
            setproctitle.setproctitle("MS-{}-{}".format(dataset[:2], seed))

            adj, features, labels = load_data(dataset)
            emb = load_emb_best_model(dataset, seed)

            preds, dur = ms_cluster(adj, emb, random_state=seed, labels=labels)
            times.append(dur)
