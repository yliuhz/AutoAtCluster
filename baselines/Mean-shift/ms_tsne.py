...
def ms_cluster(adj, features, nclass, random_state=None, labels=None):
    from community import community_louvain
    import networkx as nx
    ...
    bws = np.arange(1, max_thres+1, 1)
    for bw in bws: 
        st = time.process_time()

        cluster = MeanShift(bandwidth=bw, bin_seeding=True)
        ...
        print(bw, ari, np.unique(preds).shape)
    ...
    for model in models:
        for dataset in datasets:
            ...
            for seed in seeds:
                ...
                setproctitle.setproctitle("MS-{}-{}-{}".format(model, dataset[:2], seed))
                ...
                preds, dur = ms_cluster(adj, emb, nclass, random_state=seed, labels=labels)
                times.append(dur)
