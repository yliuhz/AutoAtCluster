...
class ML2:
    ...
    def __init__(self, graph, attributes, authorIndex, seed=None):
        self.graph = graph
        ...
        for elem, part in sorted(partition.iteritems()) :
            if(args.verbose):
                ...
                if(args.multipleDataset != None):
                    ...
                    f = open(args.dataset + "_{}_emb.2ModLouvain".format(self.seed),'a')
                f.write(out + "\n")
                f.close()
    ...
    def dist(self, v1, v2):
        ...
        # TIPS: modified with numpy
        attrV1 = np.array(list(self.attributes[v1].values()))
        attrV2 = np.array(list(self.attributes[v2].values()))
        distance = ((attrV1 - attrV2)**2).sum()
    ...
    def calculateGiniMatrixInitial(self):
        ...
        # for v1 in tqdm(self.graph, total=len(self.graph)):
...
def loadDataset(path, seed=None):
    ...
    if(not os.path.isfile(path + "_" + str(seed) + ".emb")):
        print "Error: file '" + path + ".attributes' not found"
        exit(-1)
    
    with open(path + "_" + str(seed) + ".emb") as f:
        for line in f.readlines():
    ...
    if os.path.exists(path + "_{}.2ModLouvain".format(seed)):
        os.remove(path + "_{}.2ModLouvain".format(seed))
...
def __main() :
    ...
    args.dataset = os.path.join("../data/{}".format(args.dataset), args.dataset)
    from tqdm import tqdm
    import setproctitle
    ...
    if(args.toydataset):
        ...
        if(args.multipleDataset != None):
            for i in tqdm(range(args.multipleDataset[0], args.multipleDataset[1] + 1), total=args.multipleDataset[1] + 1 - args.multipleDataset[0]):
                curDatasetIdx = i
            ...
            for seed in seeds:
                ...
                setproctitle.setproctitle("IL-{}-{}".format(filename[:2], seed))
