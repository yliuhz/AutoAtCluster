...
class Experiment:
    ...
    def train(self, args, seed):
        args.eval_freq = args.epochs

        torch.cuda.set_device(args.gpu)

        if args.gsl_mode == 'structure_refinement':
            features, nfeats, labels, nclasses, train_mask, val_mask, test_mask, adj_original = load_data(args, sparse=args.sparse)
                ...
                if epoch % args.eval_freq == 0:
                    if args.downstream_task == 'classification':
                        ...
                        if not args.sparse:
                            ...
                            np.savez("outputs/learned_adj_{}_{}_{}.npz".format(args.type_learner, args.dataset, seed), row=saved_adj.row, col=saved_adj.col, data=saved_adj.data, emb=embedding)
...
if __name__ == '__main__':
    ...
    for seed in range(args.nexp):
        ...
        setproctitle.setproctitle("SUBLIME-{}-{}".format(args.dataset, seed))
