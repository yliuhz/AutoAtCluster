...
class Experiment:
    ...
    def train(self, args, seed, rate):
        args.eval_freq = args.epochs

        torch.cuda.set_device(args.gpu)

        if args.gsl_mode == 'structure_refinement':
            features, nfeats, labels, nclasses, train_mask, val_mask, test_mask, adj_original = load_data(args, sparse=args.sparse, im=rate, seed=seed)
                ...
                if epoch % args.eval_freq == 0:
                    if args.downstream_task == 'classification':
                        ...
                        if not args.sparse:
                            ...
                            np.savez("outputs/learned_adj_{}_{}_{:.1f}_{}_5.npz".format(args.type_learner, args.dataset, rate, seed), row=saved_adj.row, col=saved_adj.col, data=saved_adj.data, emb=embedding)
...
if __name__ == '__main__':
    ...
    for rate in rates:
        for seed in range(args.nexp):
            ...
            setproctitle.setproctitle("SUBLIME-{:.1f}-{}".format(rate, seed))

            experiment = Experiment()
            experiment.train(args, seed, rate)
