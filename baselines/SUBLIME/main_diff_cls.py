...
class Experiment:
    ...
    def train(self, args, seed, nclass):
        args.eval_freq = args.epochs

        torch.cuda.set_device(args.gpu)

        if args.gsl_mode == 'structure_refinement':
            features, nfeats, labels, nclasses, train_mask, val_mask, test_mask, adj_original = load_data(args, sparse=args.sparse, diff_cls=nclass, seed=seed)
        ...
        if args.downstream_task == 'classification':
            ...
            # data_ = np.load("/data/liuyue/New/SBM/mySBM/emb_models/GGD/manual_version/outputs/GGD_{}_emb_0.npz".format(args.dataset))
            # features = torch.FloatTensor(data_["emb"])
                ...
                if epoch % args.eval_freq == 0:
                    if args.downstream_task == 'classification':
                        ...
                        for clu_trial in range(n_clu_trials):
                            kmeans = KMeans(n_clusters=nclasses, random_state=seed).fit(embedding)
                        ...
                        if args.type_learner == "fgp":
                            print("post")
                            learned_adj = graph_learner.post(learned_adj)
                        os.makedirs("outputs", exist_ok=True)
                        if not args.sparse:
                            saved_adj = learned_adj.cpu().detach().numpy()
                            saved_adj = sp.coo_matrix(saved_adj)
                            np.savez("outputs/learned_adj_{}_{}_{:d}_{}.npz".format(args.type_learner, args.dataset, nclass, seed), row=saved_adj.row, col=saved_adj.col, data=saved_adj.data, emb=embedding)

                            print(saved_adj.sum())
                            # preds = louvain_cluster(saved_adj, labels=labels.cpu().numpy(), random_state=seed)
                            ...
                            saved_adj = dgl_graph_to_torch_sparse(learned_adj).to_dense().numpy()
                            saved_adj = sp.coo_matrix(saved_adj)
                            np.savez("outputs/learned_adj_{}_{}_{:d}_{}.npz".format(args.type_learner, args.dataset, nclass, seed), row=saved_adj.row, col=saved_adj.col, data=saved_adj.data, emb=embedding)
...
if __name__ == '__main__':
    ...
    for nclass in nclasses:
        for seed in range(args.nexp):
            ...
            setproctitle.setproctitle("SUBLIME-{:d}-{}".format(nclass, seed))

            experiment = Experiment()
            experiment.train(args, seed, nclass)
