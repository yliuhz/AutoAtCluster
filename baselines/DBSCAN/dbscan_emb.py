...
def dbscan_cluster(adj, features, nclass, random_state=None, eps=None):
    from community import community_louvain
    import networkx as nx
    ...
    plt.scatter(tsne_z[:, 0], tsne_z[:, 1], c=preds)
    ...
    for model in models:
        for dataset in datasets:
            for seed in seeds:
                ...
                setproctitle.setproctitle("DBSCAN-{}-{}-{}".format(model, dataset[:2], seed))
                ...
                preds = dbscan_cluster(adj, emb, nclass, random_state=seed, eps=dbscan_params[dataset])
