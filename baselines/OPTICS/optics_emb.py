...
def optics_cluster(adj, features, nclass, random_state=None, labels=None):
    from community import community_louvain
    import networkx as nx
    ...
    minPts = np.arange(2,50,1)
    for minpt in minPts:
        cluster = OPTICS(min_samples=minpt)
    ...
    for model in models:
        for dataset in datasets:
            for seed in seeds:
                ...
                setproctitle.setproctitle("OPTICS-{}-{}-{}".format(model, dataset[:2], seed))
                ...
                preds = optics_cluster(adj, emb, nclass, random_state=seed, labels=labels)
