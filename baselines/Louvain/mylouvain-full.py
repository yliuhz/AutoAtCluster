...
class Status(object):
    ...
    def init_batch(self, emb, weight, part=None, batch_size=20000):
        """Initialize the status of a graph with every node in one community
            n: TOTAL Number of nodes
            emb: Node embeddings, n*d
            weight: weight key in networkx.graph
            part: Initial partition
            batch_size (B): Load B*n matrix into memory at one time
        """
        ...
        if part is None:
            for i in range(0, n, batch_size): # Assume node IDs from 0 to n-1 in large graph
                batch_nodeids = np.arange(i, min(i+batch_size, n))
                graph = load_complete_graph_batch_from_emb(emb, batch_nodeids)

                self_loop_weights = 0.0
                for node in batch_nodeids:
                    # print(f"{node+1}/{n}")
                    try:
                        self.node2com[node] = count
                        self_loop_weight = graph[node].get(node, {weight: 0}).get(weight)
                        deg = float(graph.out_degree(node, weight=weight)+self_loop_weight) # weighted degree. Networkx.graph use nodeids 0--B-1, global Status uses i--i+batch_size
                        self_loop_weights += self_loop_weight
                    except:
                        print(graph[node])
                        print(graph.out_degree(node, weight=weight))
                        exit(0)
                ...
                self.total_weight += (graph.size(weight=weight)-self_loop_weights) / 2 + self_loop_weights # TODO: correct?
        else:
            for i in range(0, n, batch_size):
                ...
                for node in batch_nodeids:
                    ...
                    self.node2com[node] = com
                    self_loop_weight = graph[node].get(node, {weight: 0}).get(weight)
                    deg = float(graph.out_degree(node, weight=weight)+self_loop_weight)
                    self_loop_weights += self_loop_weight
                    ...
                    for neighbor, datas in graph[node].items():
                        ...
                        if edge_weight <= 0:
                            error = "Bad graph type ({})".format(type(graph))
                            raise ValueError(error)
                        neighbor = batch_nodeids[neighbor]
                        if part[neighbor] == com:
                            if neighbor == node:
                                inc += float(edge_weight)
                            else:
...
def generate_dendrogram(emb,
    ...
    # Each embedding vector is a orthogonal basis
    ...
    status = Status()
    status.init_batch(emb, weight, part_init, batch_size)
    status_list = list()
    num_loops = __one_level(None, emb, status, weight, resolution, random_state, neighbor_mode, batch_size) #first level, use batch
    new_mod = __modularity(status, resolution) #TODO
    partition = __renumber(status.node2com) #TODO
    status_list.append(partition)
    mod = new_mod
    # current_graph = induced_graph(partition, current_graph, weight) #TODO
    current_graph = induced_graph_batch(partition, emb, weight, batch_size)
    status.init(current_graph, weight)

    while True:
        __one_level(current_graph, None, status, weight, resolution, random_state, neighbor_mode) # other levels, use origin implementation
...
def induced_graph_batch(partition, emb, weight="weight", batch_size=1024):
    ret = nx.Graph()
    ret.add_nodes_from(partition.values())
    ...
    for i in range(0, n, batch_size):
        ...
        for node1, node2, datas in graph.edges(data=True):
            if node1 > node2: # avoid duplicate add
                continue
            edge_weight = datas.get(weight, 1)
            com1 = partition[node1]
            com2 = partition[node2]
...
def __one_level(graph0, emb, status, weight_key, resolution, random_state, neighbor_mode="all", batch_size=None):
    ...
    assert graph0 is None or emb is None
    ...
    while modified and nb_pass_done != __PASS_MAX:
        ...
        if emb is None:
            randomized_node_sequence = __randomize(graph0.nodes(), random_state)
        else:
            n = len(emb)
            randomized_node_sequence = __randomize(np.arange(n), random_state)
        n = len(randomized_node_sequence)

        if graph0 is None:
            graph = load_complete_graph_batch_from_emb(emb, randomized_node_sequence[:batch_size])
            current_window=batch_size
        else:
            graph = graph0
            current_window = n+1 # Never reload graph
            
        if neighbor_mode == "all":
            for nodeidx, node in enumerate(randomized_node_sequence):
                if nodeidx >= current_window:
                    # print(f"load once {nodeidx} {current_window}")
                    graph = load_complete_graph_batch_from_emb(emb, randomized_node_sequence[current_window:min(current_window+batch_size, n)])
                    current_window += batch_size
                ...
                if best_increase > __MIN:
                    ...
                    if best_com != com_node:
                        # print(f"{node}: {com_node} --> {best_com} {best_increase:.2f}")
                        modified = True
                else:
            ...
            for nodeidx, node in enumerate(randomized_node_sequence):
                if nodeidx >= current_window:
                    batch_nodeids = randomized_node_sequence[current_window:min(current_window+batch_size, n)]
                    graph = load_complete_graph_batch_from_emb(emb, batch_nodeids)
                    current_window += batch_size
                ...
                # r_neighbor = list(graph[node].keys())[r]

                neighbors = __randomize(graph[node].keys(), random_state)
                r_neighbor = neighbors[0]
                if r_neighbor == node:
                    if len(neighbors) > 1:
            ...
            q = randomized_node_sequence # Initialize a queue
            ...
            while len(q) > 0:
                if emb is not None:
                    pop_cnt += 1
                    if pop_cnt >= current_window:
                        graph = load_complete_graph_batch_from_emb(emb, q[0:min(batch_size, len(q))])
                        current_window += min(batch_size, len(q))
    ...
    # print(f"{neighbor_mode} Num loops: {num_loops}")
    return num_loops
...
def load_complete_graph_batch(dataset, nodeids, seed):
    """dataset in cora,citeseer
        nodeids: Node IDs in batch similarity matrix
    """
    emb_path = f"/home/yliumh/github/AutoAtCluster/emb_models/GGD/manual_version/outputs/GGD_{dataset}_emb_{seed}.npz"
    try:
        data = np.load(emb_path)
        ...
        batch_emb = emb[nodeids]
        sim = torch.mm(batch_emb, emb.t())
        sim = F.relu(sim).numpy()
        sim_row = np.repeat(nodeids, n) # 000111222
        sim_col = np.tile(np.arange(n), b) #012012012
        sim_data = sim.flatten()
        sim = sp.coo_matrix((sim_data, (sim_row, sim_col)), shape=(n,n)) # Incomplete graph b*n, but set shape=(n,n) here to create networkx.graph
        graph = nx.from_scipy_sparse_array(sim, create_using=nx.DiGraph) # Use Directed graph to save memory?

        return graph
    except Exception as e:
        print(e)

def load_complete_graph_batch_from_emb(emb, nodeids):
    try:
        emb = torch.FloatTensor(emb)
        emb = F.normalize(emb)
...
def louvain_cluster(emb, labels, random_state=None, neighbor_mode="all", batch_size=20000):
    partition = best_partition(emb, random_state=random_state, neighbor_mode=neighbor_mode, batch_size=batch_size) # Use louvain defined in this file
    preds = list(partition.values())
    ...
    for neighbor_mode in neighbor_modes:
        for dataset in datasets:
            for seed in seeds:
                ...
                setproctitle.setproctitle("LouvainFull-{}-{}-{}".format(dataset, neighbor_mode, seed))
                ...
                try:
                    ...
                    alg_st = time.time()
                    preds = louvain_cluster(emb, labels, random_state=seed, neighbor_mode=neighbor_mode, batch_size=1024) #TODO: 有bug，输出全是0(batch_size=50)
                    alg_end = time.time()
