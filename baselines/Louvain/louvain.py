...
if __name__ == "__main__":
    ...
    for dataset in datasets:
        for seed in seeds:
            ...
            setproctitle.setproctitle("Louvain-{}-{}".format(dataset, seed))
            ...
            edges = np.arange(11*m, 31*m, m, dtype=int)

            for m2 in edges:
                if os.path.exists("outputs/Louvain_{}_{}_{:.0f}.npz".format(dataset, seed, m2/m)):
                ...
                try:
                    ...
                    logger.info(f"START: {dataset} {seed} {m2/m}")
                    preds = louvain_cluster(knn_adj, labels, random_state=seed)
                    logger.info(f"END: {dataset} {seed} {m2/m}")

                    os.makedirs("outputs", exist_ok=True)
                    np.savez("outputs/Louvain_{}_{}_{:.0f}.npz".format(dataset, seed, m2/m), preds=preds, labels=labels)
