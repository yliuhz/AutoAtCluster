...
def generate_dendrogram(graph,
    ...
    status_list = list()
    print(f"DEBUG: {list(status.node2com.values())}")
    print(f"DEBUG: {list(status.degrees.values())}")
    print(f"DEBUG: {list(status.gdegrees.values())}")
    print(f"DEBUG: {list(status.internals.values())}")
    print(f"DEBUG: {list(status.loops.values())}")
    print(f"DEBUG: {status.total_weight}")
...
def __one_level(graph, status, weight_key, resolution, random_state, neighbor_mode="all"):
    ...
    # print(f"DEBUG: {nx.to_numpy_array(graph)}")
    # print(f"DEBUG: {nx.to_numpy_array(graph).sum(1)}")
    # print(f"DEBUG: {graph.degree(0, weight=weight_key)}")
    ...
    for neighbor_mode in neighbor_modes:
        for dataset in datasets:
            for seed in seeds:
                ...
                if os.path.exists("outputs/myLouvainFull/Louvain_{}_{}_{}.npz".format(dataset, seed, neighbor_mode)):
                ...
                try:
                    ...
                    data = np.load(emb_path)
                    emb = data["emb"][:debug_n]
                    emb = torch.FloatTensor(emb)
                    emb = F.normalize(emb)
                    adj = torch.mm(emb, emb.t())
                    adj = F.relu(adj).numpy()
                    ...
                    alg_st = time.time()
                    preds = louvain_cluster(adj, labels, random_state=seed, neighbor_mode=neighbor_mode)
                    alg_end = time.time()
                    print(preds[:debug_n])
                    print(NMI(labels[:debug_n], preds[:debug_n]), ARI(labels[:debug_n], preds[:debug_n]))
                    ...
                    preds = list(community_louvain.best_partition(graph, random_state=seed).values())
                    print(preds[:debug_n])
                    print(NMI(labels[:debug_n], preds[:debug_n]), ARI(labels[:debug_n], preds[:debug_n]))

                    print("=========")
                    print("Labels")
                    print(labels[:debug_n])
                    ...
                    # with open("timeFull.txt", "a+") as f:
                    #     f.write(f"{dataset}\t{seed}\t{neighbor_mode}\t{alg_end-alg_st}\n")

                    # os.makedirs("outputs/myLouvainFull", exist_ok=True)
                    # np.savez("outputs/myLouvainFull/Louvain_{}_{}_{}.npz".format(dataset, seed, neighbor_mode), preds=preds, labels=labels)
                except Exception as e:
