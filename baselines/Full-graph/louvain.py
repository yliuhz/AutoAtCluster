...
if __name__ == "__main__":
    ...
    for seed in tqdm(seeds, total=len(seeds)):
        ...
        setproctitle.setproctitle("Full-{}-{}".format(dataset[:2], seed))
        ...
        emb = load_emb(args.model, args.dataset, seed)
        emb = normalize(emb)
        adj = np.matmul(emb, np.transpose(emb))
        ...
        np.savez("Cluster/{}/lo_{}_preds_{}.npz".format(args.model, args.dataset, seed), preds=preds)

        times1.append(ed1-st)
        times2.append(ed2-ed1)

    with open("time.txt", "a+") as f:
        f.write("Louvain-full {} {}\n".format(args.model, args.dataset))
        for t in times1:
            f.write("{:.3f} ".format(t))
        f.write("\n")
        for t in times2:
