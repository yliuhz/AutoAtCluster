...
def leiden_cluster(adj):
    ...
    alg_st = time.time()
    algo = nk.community.ParallelLeiden(graph_nk, iterations=32) # same as louvain
...
"""
def leiden_cluster_unattributed(datapath, graphFormat):
    graph_nk = nk.readGraph(datapath, graphFormat)

    alg_st = time.time()
    algo = nk.community.ParallelLeiden(graph_nk, iterations=32)
    plmCommunities = nk.community.detectCommunities(graph_nk, algo=algo)
    alg_ed = time.time()
...
if __name__ == "__main__":
    ...
    if args.unattr:
        for seed in seeds:
            ...
            if os.path.exists("outputs/{}/Louvain_{}_{}.npz".format(expdir, dataset, seed)):
                logger.info(f"Skip: unattr_adj_{dataset}_{seed}.npz")
                continue
            ...
            try:
                ...
                func_st = time.time()
                preds, alg_time = leiden_cluster_unattributed(args.datapath, dataformat)
                func_end = time.time()
                ...
                with open(f"outputs/{expdir}/time.txt", "a+") as f:
                    f.write(f"{dataset}\t{seed}\t{alg_time}\t{func_end-func_st}\t{time_now}\n")
                    np.savez("outputs/{}/Louvain_{}_{}.npz".format(expdir, dataset, seed), preds=preds)

                    
            except Exception as e:
                logger.error(f"Error: unattr_adj_{dataset}_{seed}.npz {e}")
            else:
                logger.info(f"Success: unattr_adj_{dataset}_{seed}.npz")
        ...
        for dataset in datasets:
            ...
            for m2 in edges:
                for seed in seeds:
                    logger.info(f"{dataset}, {m2/m:.0f}, {seed}")

                    np.random.seed(seed)
                    random.seed(seed)
                    nk.engineering.setSeed(seed, False)

                    setproctitle.setproctitle("LouvainNK-{}-{}-{:.0f}".format(dataset, m2/m, seed))

                    if os.path.exists("outputs/{}/Louvain_{}_{}_{:.0f}.npz".format(expdir, dataset, seed, m2/m)):
                    ...
                    try:
                        ...
                        func_st = time.time()
                        preds, alg_time = leiden_cluster(knn_adj)
                        func_end = time.time()
                        ...
                        os.makedirs(f"outputs/{expdir}", exist_ok=True)
                        with open(f"outputs/{expdir}/time.txt", "a+") as f:
                            f.write(f"{dataset}\t{seed}\t{m2/m:.0f}\t{alg_time}\t{func_end-func_st-alg_time}\t{time_now}\n")
                        np.savez("outputs/{}/Louvain_{}_{}_{:.0f}.npz".format(expdir, dataset, seed, m2/m), preds=preds, labels=labels)


                    except Exception as e:
                        logger.error(f"Error: knn_adj_{dataset}_{seed}_{m2/m:.0f}.npz {e}")
                    else:
                        logger.info(f"Success: knn_adj_{dataset}_{seed}_{m2/m:.0f}.npz")
